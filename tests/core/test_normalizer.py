...
class TestBasicFunctionality:
    def test_init_default(self):
        norm = mbrl.util.normalization.Normalizer(5, torch.device(_DEVICE))
    ...
    def test_init_double_precision(self):
        norm = mbrl.util.normalization.Normalizer(
    ...
    def test_update_stats_basic(self):
        norm = mbrl.util.normalization.Normalizer(2, torch.device(_DEVICE))
        data = torch.tensor(
    ...
    def test_denormalize_basic(self):
        ...
        norm = mbrl.util.normalization.Normalizer(3, torch.device(_DEVICE))
    ...
    def test_normalize_float_input(self):
        norm = mbrl.util.normalization.Normalizer(1, torch.device(_DEVICE))
...
class TestBufferIntegrity:
    def test_buffers_in_state_dict(self):
        norm = mbrl.util.normalization.Normalizer(4, torch.device(_DEVICE))
    ...
    def test_state_dict_round_trip(self):
        norm1 = mbrl.util.normalization.Normalizer(3, torch.device(_DEVICE))
        data = torch.randn(50, 3)
        norm1.update_stats(data)

        norm2 = mbrl.util.normalization.Normalizer(3, torch.device(_DEVICE))
...
class TestSaveLoad:
    def test_save_load_torch_format(self):
        norm1 = mbrl.util.normalization.Normalizer(4, torch.device(_DEVICE))
        data = torch.randn(50, 4)
        norm1.update_stats(data)

        with tempfile.TemporaryDirectory() as tmpdir:
            norm1.save(tmpdir)
            norm2 = mbrl.util.normalization.Normalizer(4, torch.device(_DEVICE))
            norm2.load(tmpdir)
...
class TestDeviceDtype:
    ...
    def test_mps_float64_downcast(self):
        norm = mbrl.util.normalization.Normalizer(2, torch.device("mps"))
...
class TestDtypePreservation:
    ...
    def test_normalize_float32_input_float32_normalizer(self):
        norm = mbrl.util.normalization.Normalizer(2, torch.device(_DEVICE), dtype=torch.float32)
    ...
    def test_normalize_float64_input_float64_normalizer(self):
        norm = mbrl.util.normalization.Normalizer(2, torch.device(_DEVICE), dtype=torch.float64)
    ...
    def test_round_trip_cross_dtype_precision(self):
        ...
        norm = mbrl.util.normalization.Normalizer(3, torch.device(_DEVICE), dtype=torch.float64)
    ...
    def test_normalize_uses_high_precision_arithmetic(self):
        ...
        norm = mbrl.util.normalization.Normalizer(1, torch.device(_DEVICE), dtype=torch.float64)
...
class TestEpsDtypeConsistency:
    ...
    def test_eps_value_float32(self):
        norm = mbrl.util.normalization.Normalizer(1, torch.device(_DEVICE), dtype=torch.float32)
        assert norm.eps.item() == pytest.approx(1e-5)
...
class TestClipRange:
    ...
    ) -> mbrl.util.normalization.Normalizer:
    ...
    def test_clip_range_stored_on_init(self):
        ...
        norm = mbrl.util.normalization.Normalizer(3, torch.device(_DEVICE), clip_range=5.0)
